 ...
 */
package ru.sfedu.hospital2.lab3.TablePerClass;
...
import ru.sfedu.hospital2.lab3.TablePerClass.HibernateUtil;
import ru.sfedu.hospital2.lab3.TablePerClass.DataProvider;
import ru.sfedu.hospital2.lab3.TablePerClass.model.Appointment;
import ru.sfedu.hospital2.lab3.TablePerClass.model.Diagnosis;
import ru.sfedu.hospital2.lab3.TablePerClass.model.Employee;
import ru.sfedu.hospital2.lab3.TablePerClass.model.Hospital;
import ru.sfedu.hospital2.lab3.TablePerClass.model.Hospitalization;
import ru.sfedu.hospital2.lab3.TablePerClass.model.MedicalInsurance;
import ru.sfedu.hospital2.lab3.TablePerClass.model.Patient;
import ru.sfedu.hospital2.lab3.TablePerClass.model.Payment;
import ru.sfedu.hospital2.lab3.TablePerClass.model.Survey;
/**
 *
 * @author Liza
 */
   ...
   /**
    ...
    public Optional<Employee> getEmployee (long id) {
        try{
            ...
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.TABLEPERCLASS_EMPLOYEE_IDENT, id));
            log.debug(String.format(Constants.TABLEPERCLASS_EMPLOYEE_IDENT, id));
    ...
    public Optional<Patient> getPatient (long id) {
        try{
             Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.TABLEPERCLASS_PATIENT_IDENT, id));
            log.debug(String.format(Constants.TABLEPERCLASS_PATIENT_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    @Override
        ...
        try {
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(String.format(Constants.TABLEPERCLASS_PASSPORT_IDENT, patient.getPassport()));          
            log.info(String.format(Constants.TABLEPERCLASS_PASSPORT_IDENT, patient.getPassport()));
            BigInteger result = (BigInteger) query.uniqueResult();
            /*if (result.compareTo(BigInteger.ZERO)!=0){
    ...
    public Optional<Appointment> getAppointment (long id) {
        try{
            Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.TABLEPERCLASS_APPOINTMENT_IDENT, id));
            log.debug(String.format(Constants.TABLEPERCLASS_APPOINTMENT_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    @Override
        ...
        try {
            Session session = getSession();
            session.beginTransaction();
            NativeQuery employee = session.createNativeQuery(String.format(Constants.TABLEPERCLASS_EMPLOYEE_IDENT, appointment.getDoctorId()));          
            NativeQuery patient = session.createNativeQuery(String.format(Constants.TABLEPERCLASS_PATIENT_IDENT, appointment.getPatientId()));          
            log.info(String.format(Constants.TABLEPERCLASS_EMPLOYEE_IDENT, appointment.getDoctorId()));
            log.info(String.format(Constants.TABLEPERCLASS_PATIENT_IDENT, appointment.getPatientId()));            
    ...
    public Optional<Survey> getSurvey (long id) {
        try{
            Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.TABLEPERCLASS_SURVEY_IDENT, id));
            log.debug(String.format(Constants.TABLEPERCLASS_SURVEY_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    @Override
        ...
        try {
            Session session = getSession();
            session.beginTransaction();
            NativeQuery employee = session.createNativeQuery(String.format(Constants.TABLEPERCLASS_EMPLOYEE_IDENT, survey.getDoctorId()));          
            NativeQuery patient = session.createNativeQuery(String.format(Constants.TABLEPERCLASS_PATIENT_IDENT, survey.getPatientId()));          
            log.info(String.format(Constants.TABLEPERCLASS_EMPLOYEE_IDENT, survey.getDoctorId()));
            log.info(String.format(Constants.TABLEPERCLASS_PATIENT_IDENT, survey.getPatientId()));
            BigInteger employeeResult = (BigInteger) employee.uniqueResult();
            BigInteger patientResult = (BigInteger) patient.uniqueResult();
    ...
    public Optional<Hospitalization> getHospitalization (long id) {
        try{
            Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.TABLEPERCLASS_HOSPITALIZATION_IDENT, id));
            log.debug(String.format(Constants.TABLEPERCLASS_HOSPITALIZATION_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    @Override
        ...
        try {
            Session session = getSession();
            session.beginTransaction();
            NativeQuery service = session.createNativeQuery(String.format(Constants.TABLEPERCLASS_SERVICE_IDENT, hospitalization.getServiceId()));          
            NativeQuery hospital = session.createNativeQuery(String.format(Constants.TABLEPERCLASS_HOSPITAL_IDENT, hospitalization.getHospitalId()));          
            log.info(Constants.TABLEPERCLASS_SERVICE_IDENT, Constants.TABLEPERCLASS_HOSPITAL_IDENT);
    ...
    public Optional<MedicalInsurance> getMedicalInsurance (long id) {
        try{
             Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.TABLEPERCLASS_MEDICALINSURANCE_IDENT, id));
            log.debug(String.format(Constants.TABLEPERCLASS_MEDICALINSURANCE_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    @Override
        ...
        try {
            Session session = getSession();
            session.beginTransaction();
            NativeQuery service = session.createNativeQuery(String.format(Constants.TABLEPERCLASS_SERVICE_IDENT, medicalInsurance.getServiceId()));          
            log.info(Constants.TABLEPERCLASS_SERVICE_IDENT);
    ...
    public Optional<Payment> getPayment (long id) {
        try{
            Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.TABLEPERCLASS_PAYMENT_IDENT, id));
            log.debug(String.format(Constants.TABLEPERCLASS_PAYMENT_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    @Override
        ...
        try {
            Session session = getSession();
            session.beginTransaction();
            NativeQuery service = session.createNativeQuery(String.format(Constants.TABLEPERCLASS_SERVICE_IDENT, payment.getServiceId()));          
            log.info(Constants.TABLEPERCLASS_SERVICE_IDENT);
    ...
    public Optional<Diagnosis> getDiagnosis (long id) {
        try{
            Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.TABLEPERCLASS_DIAGNOSIS_IDENT, id));
            log.debug(String.format(Constants.TABLEPERCLASS_DIAGNOSIS_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    @Override
        ...
        try {
            Session session = getSession();
            session.beginTransaction();
            NativeQuery service = session.createNativeQuery(String.format(Constants.TABLEPERCLASS_SURVEY_IDENT, diagnosis.getServiceId()));          
            //NativeQuery patient = session.createNativeQuery(String.format(Constants.PATIENT_IDENT, diagnosis.getPatientId()));          
            log.info(String.format(Constants.TABLEPERCLASS_SURVEY_IDENT, diagnosis.getServiceId()));
    ...
    public Optional<Hospital> getHospital (long id) {
        try{
             Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.TABLEPERCLASS_HOSPITAL_IDENT, id));
            log.debug(String.format(Constants.TABLEPERCLASS_HOSPITAL_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    public Optional<Integer> maxEmployee() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.TABLEPERCLASS_MAX_EMPLOYEE);
            log.debug(Constants.TABLEPERCLASS_MAX_EMPLOYEE);
    ...
    public Optional<Integer> maxAppointment() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.TABLEPERCLASS_MAX_APPOINTMENT);
            log.debug(Constants.TABLEPERCLASS_MAX_APPOINTMENT);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
    ...
    public Optional<Integer> maxDiagnosis() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.TABLEPERCLASS_MAX_DIAGNOSIS);
            log.debug(Constants.TABLEPERCLASS_MAX_DIAGNOSIS);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
    ...
    public Optional<Integer> maxHospital() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.TABLEPERCLASS_MAX_HOSPITAL);
            log.debug(Constants.TABLEPERCLASS_MAX_HOSPITAL);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
    ...
    public Optional<Integer> maxMedicalInsurance() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.TABLEPERCLASS_MAX_MEDICALINSURANCE);
            log.debug(Constants.TABLEPERCLASS_MAX_MEDICALINSURANCE);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
    ...
    public Optional<Integer> maxPatient() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.TABLEPERCLASS_MAX_PATIENT);
            log.debug(Constants.TABLEPERCLASS_MAX_PATIENT);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
    ...
    public Optional<Integer> maxPayment() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.TABLEPERCLASS_MAX_PAYMENT);
            log.debug(Constants.TABLEPERCLASS_MAX_PAYMENT);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
    ...
    public Optional<Integer> maxSurvey() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.TABLEPERCLASS_MAX_SURVEY);
            log.debug(Constants.TABLEPERCLASS_MAX_SURVEY);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
    ...
    public Optional<Integer> maxHospitalization() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.TABLEPERCLASS_MAX_HOSPITALIZATION);
            log.debug(Constants.TABLEPERCLASS_MAX_HOSPITALIZATION);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
