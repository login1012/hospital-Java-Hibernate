...
import ru.sfedu.hospital2.lab3.JoinedTable.HibernateUtil;
   ...
   /**
    ...
    public Optional<Employee> getEmployee (long id) {
        try{
            ...
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.EMPLOYEE_IDENT, id));
            log.debug(String.format(Constants.EMPLOYEE_IDENT, id));
    ...
    public Optional<Patient> getPatient (long id) {
        try{
             Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.PATIENT_IDENT, id));
            log.debug(String.format(Constants.PATIENT_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    @Override
        ...
        try {
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(String.format(Constants.PASSPORT_IDENT, patient.getPassport()));          
            log.info(String.format(Constants.PASSPORT_IDENT, patient.getPassport()));
            BigInteger result = (BigInteger) query.uniqueResult();
            /*if (result.compareTo(BigInteger.ZERO)!=0){
    ...
    public Optional<Appointment> getAppointment (long id) {
        try{
            Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.APPOINTMENT_IDENT, id));
            log.debug(String.format(Constants.APPOINTMENT_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    @Override
        ...
        try {
            Session session = getSession();
            session.beginTransaction();
            NativeQuery employee = session.createNativeQuery(String.format(Constants.EMPLOYEE_IDENT, appointment.getDoctorId()));          
            NativeQuery patient = session.createNativeQuery(String.format(Constants.PATIENT_IDENT, appointment.getPatientId()));          
            log.info(String.format(Constants.EMPLOYEE_IDENT, appointment.getDoctorId()));
            log.info(String.format(Constants.PATIENT_IDENT, appointment.getPatientId()));            
    ...
    public Optional<Survey> getSurvey (long id) {
        try{
            Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.SURVEY_IDENT, id));
            log.debug(String.format(Constants.SURVEY_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    @Override
        ...
        try {
            Session session = getSession();
            session.beginTransaction();
            NativeQuery employee = session.createNativeQuery(String.format(Constants.EMPLOYEE_IDENT, survey.getDoctorId()));          
            NativeQuery patient = session.createNativeQuery(String.format(Constants.PATIENT_IDENT, survey.getPatientId()));          
            log.info(String.format(Constants.EMPLOYEE_IDENT, survey.getDoctorId()));
            log.info(String.format(Constants.PATIENT_IDENT, survey.getPatientId()));
            BigInteger employeeResult = (BigInteger) employee.uniqueResult();
            BigInteger patientResult = (BigInteger) patient.uniqueResult();
    ...
    public Optional<Hospitalization> getHospitalization (long id) {
        try{
            Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.HOSPITALIZATION_IDENT, id));
            log.debug(String.format(Constants.HOSPITALIZATION_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    @Override
        ...
        try {
            ...
            NativeQuery hospital = session.createNativeQuery(String.format(Constants.HOSPITAL_IDENT, hospitalization.getHospitalId()));          
            log.info(Constants.SERVICE_IDENT, Constants.HOSPITAL_IDENT);
    ...
    public Optional<MedicalInsurance> getMedicalInsurance (long id) {
        try{
             Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.MEDICALINSURANCE_IDENT, id));
            log.debug(String.format(Constants.MEDICALINSURANCE_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    @Override
        ...
        try {
            Session session = getSession();
            session.beginTransaction();
            NativeQuery service = session.createNativeQuery(String.format(Constants.SERVICE_IDENT, medicalInsurance.getServiceId()));          
            log.info(Constants.SERVICE_IDENT);
    ...
    public Optional<Payment> getPayment (long id) {
        try{
            Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.PAYMENT_IDENT, id));
            log.debug(String.format(Constants.PAYMENT_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    @Override
        ...
        try {
            Session session = getSession();
            session.beginTransaction();
            NativeQuery service = session.createNativeQuery(String.format(Constants.SERVICE_IDENT, payment.getServiceId()));          
            log.info(Constants.SERVICE_IDENT);
    ...
    public Optional<Diagnosis> getDiagnosis (long id) {
        try{
            Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.DIAGNOSIS_IDENT, id));
            log.debug(String.format(Constants.DIAGNOSIS_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    @Override
        ...
        try {
            Session session = getSession();
            session.beginTransaction();
            NativeQuery service = session.createNativeQuery(String.format(Constants.SURVEY_IDENT, diagnosis.getServiceId()));          
            //NativeQuery patient = session.createNativeQuery(String.format(Constants.PATIENT_IDENT, diagnosis.getPatientId()));          
            log.info(String.format(Constants.SURVEY_IDENT, diagnosis.getServiceId()));
    ...
    public Optional<Hospital> getHospital (long id) {
        try{
             Session identSession = getSession();
            identSession.beginTransaction();
            NativeQuery query = identSession.createNativeQuery(String.format(Constants.HOSPITAL_IDENT, id));
            log.debug(String.format(Constants.HOSPITAL_IDENT, id));
            BigInteger result = (BigInteger) query.uniqueResult();
            if (result.compareTo(BigInteger.ZERO)==0){
                log.error(Constants.ERROR);
                return Optional.empty();
    ...
    public Optional<Integer> maxEmployee() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.MAX_EMPLOYEE);
            log.debug(Constants.MAX_EMPLOYEE);
    ...
    public Optional<Integer> maxAppointment() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.MAX_APPOINTMENT);
            log.debug(Constants.MAX_APPOINTMENT);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
    ...
    public Optional<Integer> maxDiagnosis() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.MAX_DIAGNOSIS);
            log.debug(Constants.MAX_DIAGNOSIS);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
    ...
    public Optional<Integer> maxHospital() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.MAX_HOSPITAL);
            log.debug(Constants.MAX_HOSPITAL);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
    ...
    public Optional<Integer> maxMedicalInsurance() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.MAX_MEDICALINSURANCE);
            log.debug(Constants.MAX_MEDICALINSURANCE);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
    ...
    public Optional<Integer> maxPatient() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.MAX_PATIENT);
            log.debug(Constants.MAX_PATIENT);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
    ...
    public Optional<Integer> maxPayment() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.MAX_PAYMENT);
            log.debug(Constants.MAX_PAYMENT);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
    ...
    public Optional<Integer> maxSurvey() {
        try{
            Session session = getSession();
            session.beginTransaction();
            NativeQuery query = session.createNativeQuery(Constants.MAX_SURVEY);
            log.debug(Constants.MAX_SURVEY);
            BigInteger id = (BigInteger) query.uniqueResult();
            //Long id = (Long) query.uniqueResult();
            int max;
            if (id != null){
