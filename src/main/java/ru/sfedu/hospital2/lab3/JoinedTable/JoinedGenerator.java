...
import ru.sfedu.hospital2.lab3.JoinedTable.HibernateDataProvider;
 ...
 * @author Liza
 */
public class JoinedGenerator {
    private static final Logger log = LogManager.getLogger(JoinedGenerator.class);
    ...
    public  List<Employee> generateEmployee() {
     ...
     public  List<Appointment> generateAppointment() {
        ...
        for (int i = 0; i < 2; i++) {
            Appointment appointment = new Appointment();
                ...
                NativeQuery employeeQuery = employeeSession.createNativeQuery(Constants.EMPLOYEE);
     ...
     public  List<Diagnosis> generateDiagnosis() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            diagnosis.setSignature(false);            
                ...
                NativeQuery serviceQuery = serviceSession.createNativeQuery(Constants.SURVEY);
     ...
     public  List<Hospitalization> generateHospitalization() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            Session serviceSession = provider.getSession();
                serviceSession.beginTransaction();
                NativeQuery serviceQuery = serviceSession.createNativeQuery(Constants.APPOINTMENT);
                List<BigInteger>service = serviceQuery.getResultList();
                ...
                NativeQuery hospitalQuery = hospitalSession.createNativeQuery(Constants.HOSPITAL);
